    ...
    <div class="space-y-5 profile-page mx-auto ">
        ...
        <div class="grid grid-cols-8 gap-5">
            ...
            <div class="card col-span-3">
                ...
                <header class="card-header cust-card-header noborder">
                    <iconify-icon wire:loading wire:target="searchRm" class="loading-icon text-lg"
                        icon="line-md:loading-twotone-loop"></iconify-icon>
                    <input type="text" class="form-control !pl-9 mr-1 basis-1/4" placeholder="Search here..."
                        wire:model.live.debounce.400ms="searchRm">
                </header>
                ...
                <div class="card-body px-6 pb-6  overflow-x-auto">
                    <div class=""> <!-- Add this wrapper to allow horizontal scroll -->
                        ...
                        @foreach ($rawMaterials as $rm)
                            <div class="card-body flex flex-col justify-between border rounded-lg h-full menu-open p-0 mb-5"
                                style="border-color:rgb(224, 224, 224)">
                                <div class="break-words items-center my-1 m-4">

                                    <h5 class="capitalize py-3 flex text-center">
                                        <a href="{{ route('material.show', $rm->id) }}"> <span
                                                class="hover-underline text-center">
                                                {{ $rm->name }}
                                            </span>
                                        </a>
                                    </h5>
                                    <p class="text-xs">Minimum Limit: <b>{{ $rm->min_limit }}</b></p>
                                </div>
                                <hr><br>
                                <div class=" mb-4 text-center">

                                    <div class="border-r ml-5 col-span-3 md:col-span-1 mb-3 md:mb-0">
                                        @if ($rm->inventory->on_hand > $rm->min_limit)
                                            <h5>{{ $rm->inventory->on_hand }}</h5>
                                            <p class="text-sm mt-2">
                                                <span
                                                    class="badge bg-success-500 text-dark-500 bg-opacity-50 capitalize">
                                                    <iconify-icon icon="icon-park-outline:dot" width="1.2em"
                ...
                <div style="position: sticky ; bottom:0;width:100%; z-index:10;"
                    class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                    {{ $rawMaterials->links('vendor.livewire.simple-bootstrap') }}
                </div>
