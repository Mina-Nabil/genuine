...
class Order extends Model
{
            ...
            ->when(!empty($deliveryDates), function ($query) use ($deliveryDates) {
                foreach ($deliveryDates as $dd) {
                    $query->where(function ($q) use ($dd) {
                        ...
                        $q->where('delivery_date', '>=', $dateObj->format('Y-m-d 00:00:00'))
                            ->where('delivery_date', '<=', $dateObj->format('Y-m-d 23:59:59'));
                    });
                }
            })
    ...
    public function scopeWeeklyZoneReport($query, $fromDate, $toDate, $searchText = null, $zoneIds = [])
    {
        ...
        $query
            ...
            ->selectRaw('COALESCE(SUM(products.weight * order_products.quantity), 0) as total_weight')
